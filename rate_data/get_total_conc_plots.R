...
par(mfrow=c(2, 5), mar=c(4,2,1,1), new=F)
for (i in 1: imax) {
    file <- files[i]
    name <- names[i]
    ...
    bot <- min(totals) #signif(min(totals), 2)
    top <- max(totals) #signif(max(totals), 2)
    plot(totals, xlab=name, xaxt='n', yaxp=c(bot, top, 1), ylim=c(bot, top), type='l')
