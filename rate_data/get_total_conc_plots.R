...
par(mfrow=c(2, 5), mar=c(4,1,1,2), new=F)
for (i in 1: imax) {
    file <- files[i]
    name <- names[i]
    ...
    bot <- floor(min(totals)) #signif(min(totals), 2)
    top <- floor(max(totals)) #signif(max(totals), 2)
    plot(totals, xlab=name, xaxt='n', yaxt='n', ylim=c(bot, top), type='l', bty='o')
    axis(side=4, yaxp=c(bot, top, 1), ylim=c(bot, top), las=1)
